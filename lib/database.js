...
function save_tx(txid, cb) {
  lib.get_rawtransaction(txid, false, function(rawtx){
    lib.get_rawtransaction(txid, true, function(tx){
...
module.exports = {
  ...
  get_txs: function({ limit, offset, starttimestamp }, cb) {
    let criteria = {};

    if (starttimestamp) {
      criteria.timestamp = { $gte : starttimestamp };
    }
  ...
  get_addr_txs: function(addr, { limit, offset, starttimestamp }, cb) {
    let criteria = { $or: [ { 'vout.addresses': addr }, { 'vin.addresses': addr } ] };
  ...
  count_addr_txs: function(addr, cb) {
    Tx.count({ $or: [ { 'vout.addresses': addr }, { 'vin.addresses': addr } ] })
  ...
  update_tx_db: function(coin, start, end, timeout, cb) {
    var complete = false;
    var bulkTxs = Tx.collection.initializeUnorderedBulkOp();
    var bulkAddrs = Address.collection.initializeUnorderedBulkOp();
    var startTime = Date.now();

    lib.syncLoop((end - start) + 1, function (loop) {
      var x = loop.iteration();
      var blockindex = start + x;

      lib.get_blockhash(blockindex, function(blockhash){
        if (!blockhash) {
          console.log('Error indexing block, height: ' + blockindex);
          return loop.next();
        }

        console.log('Indexing block, height: ' + blockindex + ', hash: ' + blockhash);

        lib.get_block(blockhash, function(block) {
          if (!block || typeof (block) === 'string') {
            console.log('Error indexing block, height: ' + blockindex + ', hash: ' + blockhash);
            return loop.next();
          }
            ...
            Tx.findOne({ txid: txid }, function(err, tx) {
              ...
              lib.get_rawtransaction(txid, false, function(rawtx) {
                lib.get_rawtransaction(txid, true, function(tx){
                  ...
                  lib.prepare_vin(tx, function(vin) {
                    lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
                      var total = vout.reduce((acc, x) => acc + x.amount, 0);
                      ...
                      vout.forEach(x => bulkAddrs.find({ a_id: x.addresses }).upsert().update({
                        $inc: {
                          received: x.amount,
                          balance: x.amount
                        }
                      ...
                      nvin.forEach(x => bulkAddrs.find({ a_id: x.addresses }).upsert().update({
                        $inc: {
                          sent: x.amount,
                          balance: -x.amount
                        }
                      ...
                      console.log('Added tx '+txid+' to bulk (' + bulkTxs.length + '/' + settings.bulk_size_save + ')');

                      if (bulkTxs.length >= settings.bulk_size_save) {
                        bulkTxs.execute(function (err, result) {
                          bulkTxs = Tx.collection.initializeUnorderedBulkOp();
                          console.log('Saved txs bulk');

                          bulkAddrs.execute(function (err, res) {
                            console.log('Saved addrs bulk');
                            bulkAddrs = Address.collection.initializeUnorderedBulkOp();

                            console.log('Saved ' + settings.bulk_size_save + ' in ' + (Date.now() - startTime));
                            startTime = Date.now();

                            Stats.update({ coin: coin }, {
                              last: blockindex
                            }, function() {
                              console.log('Saved last index. Moving to next portion.');
                              subloop.next();
                            });
                          });
                        });
        ...
        bulkTxs.execute(function (err, result) {
          ...
          bulkAddrs.execute(function (err, res) {
            ...
            Stats.update({ coin: coin }, {
              ...
              console.log('Saved last block index');
              console.log('Indexed all blocks');
              cb();
            });
          });
        });
