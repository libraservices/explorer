...
module.exports = {
  ...
  get_rawtransaction: function(hash, decode, cb) {
    var uri = base_url + 'getrawtransaction?txid=' + hash + '&decrypt=' + (decode ? 1 : 0);
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },
  ...
  get_input_addresses: function(input, vout, cb) {
    ...
    if (input.coinbase) {
      ...
      module.exports.get_rawtransaction(input.txid, true, function(tx){
        if (tx) {
